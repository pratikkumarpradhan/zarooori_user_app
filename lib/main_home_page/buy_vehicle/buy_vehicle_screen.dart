...
class BuyVehicleScreen extends StatefulWidget {
  ...
  const BuyVehicleScreen({super.key, this.vehicleCatId = '1'});
...
class _BuyVehicleScreenState extends State<BuyVehicleScreen> {
  ...
  void _onSearch() async {
    ...
    try {
      ...
      if (_brandsList.isNotEmpty && _brandIndex < _brandsList.length) {
        ...
        if (brand.vehicle_brand_id != null) buyVehicle.vehicle_brand.add(brand.vehicle_brand_id!);
        final types = brand.type_list;
        if (types.isNotEmpty && _typeIndex < types.length) {
          final t = types[_typeIndex];
          if (t.vehicle_type_id != null) buyVehicle.vehicle_type.add(t.vehicle_type_id!);
          final models = t.model_list;
          if (models.isNotEmpty && _modelIndex < models.length) {
            final m = models[_modelIndex];
            if (m.vehicle_model_id != null) buyVehicle.vehicle_model.add(m.vehicle_model_id!);
          }
        }
      }
      if (_yearsList.isNotEmpty && _yearIndex < _yearsList.length) {
        final y = _yearsList[_yearIndex];
        if (y.id != null) buyVehicle.vehicle_year.add(y.id!);
      }
      if (_fuelList.isNotEmpty && _fuelIndex < _fuelList.length) {
        final f = _fuelList[_fuelIndex];
        if (f.id != null) buyVehicle.vehicle_fuel.add(f.id!);
      }
      if (_cityList.isNotEmpty && _cityIndex < _cityList.length) {
        final c = _cityList[_cityIndex];
        if (c.id != null && c.id != '0') buyVehicle.city_id = c.id;
      }

      if (!mounted) return;
      Get.to(() => SearchVehicleListScreen(
            vehicleCatId: widget.vehicleCatId,
            buyVehicleFilter: buyVehicle,
          ));
    } catch (e) {
      if (mounted) {
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            TextButton(
                                              ...
                                              child: Text(
                                                'Retry',
                                                style: AppTextStyles.textView(size: 13, color: PremiumDecorations.primaryButton).copyWith(
                                                  fontWeight: FontWeight.w600,
                                                ),
                                              ),
                                            ),
  ...
  Widget _buildBrandDropdown() {
    final val = _brandsList.isEmpty || _brandIndex >= _brandsList.length
        ? ''
  ...
  Widget _buildYearDropdown() {
    final val = _yearsList.isEmpty || _yearIndex >= _yearsList.length
        ? ''
  ...
  Widget _buildFuelDropdown() {
    final val = _fuelList.isEmpty || _fuelIndex >= _fuelList.length
        ? ''
        : (_fuelList[_fuelIndex].fuel ?? '');
    return _buildDropdown(
      value: val,
      hint: 'Select Fuel',
      onTap: () => _showPicker(
  ...
  Widget _buildStateDropdown() {
    final val = _stateList.isEmpty || _stateIndex >= _stateList.length
        ? 'All States'
  ...
  Widget _buildCityDropdown() {
    final val = _cityList.isEmpty || _cityIndex >= _cityList.length
        ? 'All Cities'
