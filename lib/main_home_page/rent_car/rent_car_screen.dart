...
class RentCarScreen extends StatefulWidget {
  ...
  const RentCarScreen({super.key, required this.productRequest});
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      final req = ProductRequest(
        master_category_id: widget.productRequest.master_category_id ?? '11',
        vehicle_category_id: widget.productRequest.vehicle_category_id,
        city_id: cityId,
      );
      final list = await VehicleInsuranceApi.getProductList(req);
      if (!mounted) return;
      setState(() {
        _products = list;
          ...
          IconButton(
            ...
            onPressed: () async {
              final req = await Get.to<ProductRequest>(() => SearchProductScreen(
                    mainCatId: '11',
                    screenTitle: 'Search Cars For Rent',
                    initialRequest: widget.productRequest,
                  ));
              if (req != null && mounted) {
                Get.off(() => RentCarScreen(productRequest: req));
              }
            },
          ),
        ],
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Search by Company, model',
