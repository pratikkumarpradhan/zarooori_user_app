...
class _CourierServicesScreenState extends State<CourierServicesScreen> {
  ...
  List<CompanyDetails> _freelancers = [];
  List<CompanyDetails> _filteredCompanies = [];
  List<CompanyDetails> _filteredFreelancers = [];
  ...
  static const String _masterCategoryId = '12';
  ...
  Future<void> _loadCompanies() async {
    ...
    try {
      ...
      final req = ProductRequest(master_category_id: _masterCategoryId, city_id: cityId);
      final list = await VehicleInsuranceApi.getCompanyList(req);
      if (!mounted) return;
      final companies = <CompanyDetails>[];
      final freelancers = <CompanyDetails>[];
      for (final c in list) {
        final type = c.company_type ?? '0';
        if (type == '1') {
          freelancers.add(c);
        } else {
          companies.add(c);
      ...
      setState(() {
        ...
        _filteredFreelancers = List.from(freelancers);
        _isLoading = false;
      });
    } catch (e) {
      if (mounted) {
  ...
  void _onSearchChanged() {
    ...
    if (q.isEmpty) {
      ...
      setState(() {
        _filteredCompanies = _companies.where((c) => _matchesSearch(c, q)).toList();
        _filteredFreelancers = _freelancers.where((c) => _matchesSearch(c, q)).toList();
      });
    }
  }
  ...
  void _onCompanyTap(CompanyDetails company) {
    Get.to(() => CompanyDetailScreen(
          ...
          mainCatId: _masterCategoryId,
          courierDetails: CourierDetails(
            seller_id: company.seller_id,
            seller_name: company.owner_name,
  ...
  void _onSendRequest(CompanyDetails company) {
    Get.to(() => AddCourierDetailsScreen(
          courierDetails: CourierDetails(
            seller_id: company.seller_id,
            seller_name: company.owner_name,
            seller_company_id: company.id,
        ...
        children: [
            ...
            children: [
              Expanded(
                child: GestureDetector(
                  onTap: () => setState(() => _showCompanies = true),
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 12),
                    decoration: BoxDecoration(
                      color: _showCompanies ? AppColors.orange : Colors.transparent,
                    ...
                    child: Text(
                      ...
                      style: AppTextStyles.textView(
                        size: 14,
                        color: _showCompanies ? AppColors.white : AppColors.black,
                      ),
                    ),
                  ),
                ),
              ...
              Expanded(
                child: GestureDetector(
                  onTap: () => setState(() => _showCompanies = false),
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 12),
                    decoration: BoxDecoration(
                      color: !_showCompanies ? AppColors.orange : Colors.transparent,
          ...
          Expanded(
            child: _isLoading
                ...
                : _errorMsg != null && _companies.isEmpty && _freelancers.isEmpty
                    ? Center(
                        child: Column(
  ...
  Widget _buildList() {
    final list = _showCompanies ? _filteredCompanies : _filteredFreelancers;
    if (list.isEmpty) {
      return Center(
        child: Text(
          _showCompanies ? 'No companies found' : 'No freelancers found',
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) => _CompanyCard(
        company: list[i],
        onTap: () => _onCompanyTap(list[i]),
        onSendRequest: () => _onSendRequest(list[i]),
      ),
    );
  }
}
